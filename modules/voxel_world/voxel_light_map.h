...
public:
	...
	struct NeighborData {
		const uint16_t *blocks_px = nullptr;
		const uint16_t *blocks_nx = nullptr;
		const uint16_t *blocks_pz = nullptr;
		const uint16_t *blocks_nz = nullptr;
	...
	// p_opacity / p_emission are flat cache arrays from VoxelBlockRegistry.
	static void propagate_all(uint8_t *p_light_data, const uint16_t *p_blocks, const NeighborData &p_neighbors, const uint8_t *p_opacity, const uint8_t *p_emission);

	// Sunlight only.
	static void propagate_sunlight(uint8_t *p_light_data, const uint16_t *p_blocks, const NeighborData &p_neighbors, const uint8_t *p_opacity);

	// Block (emissive) light only.
	static void propagate_block_light(uint8_t *p_light_data, const uint16_t *p_blocks, const NeighborData &p_neighbors, const uint8_t *p_opacity, const uint8_t *p_emission);
	...
	static void remove_and_repropagate_sunlight(uint8_t *p_light_data, const uint16_t *p_blocks, const NeighborData &p_neighbors, const uint8_t *p_opacity);
	static void remove_and_repropagate_block_light(uint8_t *p_light_data, const uint16_t *p_blocks, const NeighborData &p_neighbors, const uint8_t *p_opacity, const uint8_t *p_emission);
...
private:
	...
	static _FORCE_INLINE_ int get_block(const uint16_t *p_blocks, int p_x, int p_y, int p_z, const NeighborData &p_nb) {
		if (p_y < 0 || p_y >= CY) {
			return VOXEL_BLOCK_AIR;
		}
		if (p_x < 0) {
			return p_nb.blocks_nx ? (int)p_nb.blocks_nx[VoxelTerrainGenerator::block_index(p_x + CX, p_y, p_z)] : VOXEL_BLOCK_AIR;
		}
		if (p_x >= CX) {
			return p_nb.blocks_px ? (int)p_nb.blocks_px[VoxelTerrainGenerator::block_index(p_x - CX, p_y, p_z)] : VOXEL_BLOCK_AIR;
		}
		if (p_z < 0) {
			return p_nb.blocks_nz ? (int)p_nb.blocks_nz[VoxelTerrainGenerator::block_index(p_x, p_y, p_z + CZ)] : VOXEL_BLOCK_AIR;
		}
		if (p_z >= CZ) {
			return p_nb.blocks_pz ? (int)p_nb.blocks_pz[VoxelTerrainGenerator::block_index(p_x, p_y, p_z - CZ)] : VOXEL_BLOCK_AIR;
		}
